...
export default function AutumnGenerator() {
  ...
  async function generate() {
    setLoading(true);
    setVideos([]);
    setMessage(`Selecting products for ${videoCount} Autumn video${videoCount === 1 ? "" : "s"}...`);

    try {
      const response = await fetch("/api/generate-clothing", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ category: "Autumn", videoCount })
      });
      ...
      link.href = url;
      link.download = `autumn-outfits-${videoCount}-videos.zip`;
      ...
      setMessage(`Complete, ${videoCount} Autumn video${videoCount === 1 ? "" : "s"} with ${videoCount * 6} prompts generated.`);
    } catch (error) {
      console.error("Autumn generate error:", error);
  ...
  return (
    <section className="clothing-generator">
      <div className="clothing-hero">
        <div className="eyebrow">Autumn</div>
        <h1>Create <span>Autumn Looks.</span></h1>
        <p>Generate complete Autumn videos with 6 image prompts per video.</p>
      ...
      {videos.length > 0 && (
        <section className="clothing-results">
          <div className="results-header">
            <div><div className="section-label">GENERATED CONTENT</div><h2>Your Autumn prompts</h2></div>
