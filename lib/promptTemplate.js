const PROMPT_TEMPLATE = `
Brug det vedhæftede template som præcis baggrund og layout.

Behold templateets layout, placering, proportioner, typografi, farver og dekorative elementer så tæt på originalen som muligt.

Dette er en produktslide med præcis tre produkter.

Indsæt de tre uploadede produktbilleder i de tre billedfelter.

PRODUKTBILLEDER:

Produktbillederne skal være det visuelle fokus i deres respektive sektioner.

Gør hvert produkt tydeligt, skarpt, rent og visuelt præsentabelt.

Produktet skal placeres naturligt og centreret i billedfeltet.

Sørg for at produktet fylder en passende og tydelig del af billedfeltet uden at blive for lille.

Brug hele det tilgængelige billedfelt effektivt, så produktet er let at se og genkende.

Fjern den originale baggrund fra hvert produktbillede, så kun selve produktet vises.

Rens forsigtigt kanterne omkring produktet, så produktet fremstår professionelt og pænt isoleret.

Forbedr produktbilledets visuelle kvalitet med diskret enhancement.

Produktet skal fremstå skarpt, klart, rent og velbelyst.

Forbedr forsigtigt kontrast, lys, skarphed og detaljer, når det er nødvendigt.

Enhancement må aldrig ændre selve produktets identitet eller design.

Behold produktets originale form, farve, detaljer, materiale, tekst, mønster og udseende.

Du må ikke ændre produktets form.

Du må ikke ændre produktets farve.

Du må ikke ændre produktets detaljer.

Du må ikke ændre produktets materiale.

Du må ikke ændre produktets tekst.

Du må ikke ændre produktets design.

Du må ikke tilføje nye detaljer til produktet.

Du må ikke fjerne eksisterende detaljer fra produktet.

Du må ikke generere nye produkter.

Du må ikke erstatte de uploadede produkter med andre produkter.

Du må ikke finde på eller tilføje produkter.

Produktet skal altid være det samme fysiske produkt som på et af de uploadede billeder.

Produktbilledet må ikke beskæres på en måde, der skjuler dele af produktet.

Sørg for passende luft omkring produktet.

Undgå unødvendigt tomt område omkring produktet.

Produktet skal have en naturlig visuel størrelse i billedfeltet.

Produktet skal være i fokus og ikke baggrunden.

Fjern de stiplede linjer omkring billedfelterne.

VIGTIG IDENTIFIKATION AF PRODUKTER:

Filnavnet på hvert uploadet produktbillede fungerer som et vigtigt produkt-ID.

Filnavnet indeholder produktnummer, produktnavn og produktkode.

Formatet er:

B1-Squishy-ProduktNavn-ProduktKode.jpg

Eksempel:

B1-Squishy-Butter-HDK23D.jpg

Dette betyder:

Produktnummer: P1
Produktnavn: Butter
Produktkode: HDK23D

Brug filnavnet aktivt til at koble det fysiske produkt sammen med den korrekte produktinformation.

Match altid:

Filnavn → fysisk produkt → produktnavn → produktkode → pris.

Uploadrækkefølgen er kun en reference.

Uploadrækkefølgen må ikke bruges som eneste måde at identificere produkterne på.

Hvis billederne bliver uploadet i en anden rækkefølge end rækkefølgen i denne prompt, skal du bruge filnavnet til at identificere det korrekte produkt.

Hvis filen med navnet B1-Squishy-Butter-HDK23D.jpg bliver uploadet som det tredje billede, skal den stadig behandles som Produkt 1.

Hvis filen med navnet B2-Squishy-Apple-ABC45F.jpg bliver uploadet som det første billede, skal den stadig behandles som Produkt 2.

Filnavnet skal derfor bruges til at identificere produktet, uanset hvilken rækkefølge billederne vises i ved upload.

Brug samtidig produktets visuelle udseende til at kontrollere matchingen.

Hvis produktnavnet tydeligt beskriver produktets visuelle udseende, skal dette bruges som ekstra kontrol.

Hvis produktnavnet er "Apple Squishy", skal du kontrollere at det fysiske produkt visuelt er det æbleformede produkt blandt de uploadede billeder.

Hvis produktnavnet er "Strawberry Squishy", skal du kontrollere at det fysiske produkt visuelt er jordbærproduktet blandt de uploadede billeder.

Hvis produktnavnet er "Bear Squishy", skal du kontrollere at det fysiske produkt visuelt er bjørneproduktet blandt de uploadede billeder.

Dette er kun eksempler på identifikationslogikken.

Brug altid de faktiske produkter og de faktiske oplysninger fra denne prompt.

Hvis uploadrækkefølgen ikke passer med filnavnene, skal filnavnet og produktets faktiske visuelle identitet prioriteres.

Hvis filnavnet og produktets visuelle udseende tydeligt stemmer overens, skal dette bruges som bekræftelse af produktidentiteten.

Du må ikke ændre et produkt for at få det til at passe med filnavnet.

Du må ikke ændre produktets visuelle udseende.

Du må ikke opfinde et produkt.

Du må ikke vælge et produkt fra din egen viden.

Du må kun bruge produkter, der faktisk findes blandt de uploadede billeder.

PRODUKTINFORMATION:

Produkt 1:
Filnavn: {{PRODUCT_1_FILENAME}}
Navn: {{PRODUCT_1_NAME}}
Kode: {{PRODUCT_1_CODE}}
Pris: {{PRODUCT_1_PRICE}}

Produkt 2:
Filnavn: {{PRODUCT_2_FILENAME}}
Navn: {{PRODUCT_2_NAME}}
Kode: {{PRODUCT_2_CODE}}
Pris: {{PRODUCT_2_PRICE}}

Produkt 3:
Filnavn: {{PRODUCT_3_FILENAME}}
Navn: {{PRODUCT_3_NAME}}
Kode: {{PRODUCT_3_CODE}}
Pris: {{PRODUCT_3_PRICE}}

KRAV:

Behold præcis samme layout som template.

Behold præcis de uploadede produktbilleder som grundlag for produkterne.

Produktet skal være tydeligt, skarpt, centreret og visuelt fremhævet.

Gør produktbillederne rene og professionelle uden at ændre selve produkterne.

Fjern pladsholderen BILLEDE 1 og indsæt det fysiske produkt, der matcher filnavnet {{PRODUCT_1_FILENAME}} og Produkt 1.

Fjern pladsholderen BILLEDE 2 og indsæt det fysiske produkt, der matcher filnavnet {{PRODUCT_2_FILENAME}} og Produkt 2.

Fjern pladsholderen BILLEDE 3 og indsæt det fysiske produkt, der matcher filnavnet {{PRODUCT_3_FILENAME}} og Produkt 3.

Brug filnavnet som produkt-ID.

Brug produktets visuelle udseende som ekstra kontrol af identiteten.

Uploadpositionen må ikke overtrumfe filnavnet.

Fjern NAVN 1 og indsæt produktets rigtige navn.

Fjern NAVN 2 og indsæt produktets rigtige navn.

Fjern NAVN 3 og indsæt produktets rigtige navn.

Fjern KODE 1 og indsæt den rigtige kode.

Fjern KODE 2 og indsæt den rigtige kode.

Fjern KODE 3 og indsæt den rigtige kode.

Behold teksten Code: under produktnavnet.

Vis produktkoden i den lyserøde afrundede boks.

Vis prisen stort med rød tekst under koden.

Behold de lyserøde dekorative elementer, hjerter, streger og øvrige elementer fra templaten.

Behold hvid baggrund.

Behold ens afstand mellem alle tre produktsektioner.

Ingen ekstra overskrift.

Ingen ekstra tekst.

Ingen ekstra produkter.

Ingen ændring af baggrundsdesignet.

Ingen erstatning af produkterne.

Ingen opfundne produkter.

Ingen ændring af produktdesign.

OUTPUT:

Outputtet skal altid være præcis 9:16.

Outputformatet skal være vertikalt 9:16.

Lever som ét færdigt vertikalt katalogbillede i 9:16.

Billedet skal have solid hvid baggrund.

Billedet må ikke have gennemsigtig baggrund.

Alle tre produkter skal være tydelige og lette at se.

Alle tre produkter skal være skarpe og visuelt fremhævede.

Produktbillederne skal være pæne, rene og professionelt præsenterede.

Høj opløsning.

Klar og skarp tekst.

Produktnavne, koder og priser skal være lette at læse.

Ingen ekstra elementer uden for templateets eksisterende design.
`;

const COVER_TITLE_WORDS = [
  "Nostalgic",
  "Cute",
  "Kawaii",
  "Adorable",
  "Dreamy",
  "Cozy",
  "Satisfying",
  "Viral",
  "Trending",
  "Rare",
  "Collectible",
  "Must Have",
  "Cute Finds",
  "Hidden Gems",
  "New Finds",
  "Best Finds",
  "Mini",
  "Tiny",
  "Soft",
  "Squishy",
  "Stress Relief",
  "Fun Finds",
  "Cute Picks",
  "Favorite Finds",
  "Collector's Picks",
  "Sweet Finds",
  "Dream Collection",
  "Cute Collection",
  "Nostalgia",
  "Throwback",
  "Childhood",
  "Childhood Favorites",
  "Retro",
  "Classic",
  "Iconic",
  "Magical",
  "Whimsical",
  "Playful",
  "Lovely",
  "Charming",
  "Fun",
  "Popular",
  "Best Sellers",
  "New Collection",
  "Mini Collection",
  "Cute Collection",
  "Squishy Collection",
  "Ultimate Collection",
  "Secret Finds",
  "Viral Finds"
];

function createCoverTitle() {
  // 50% chance for kun:
  // Shein Squishy Codes
  if (Math.random() < 0.5) {
    return "Shein Squishy Codes";
  }

  // 50% chance for et tilfældigt ord fra listen
  // efterfulgt af:
  // Shein Squishy Codes
  const word =
    COVER_TITLE_WORDS[
      Math.floor(
        Math.random() *
          COVER_TITLE_WORDS.length
      )
    ];

  return `${word} Shein Squishy Codes`;
}

const COVER_PROMPT = `
Brug det vedhæftede forside template som præcis baggrund og behold hele det eksisterende design uændret.

Behold forsidens titel præcis som angivet her:

{{COVER_TITLE}}

Behold farverne, typografien, blomsterne, sløjfen, hjerterne, stregerne, baggrunden og alle øvrige dekorative elementer uændret.

Der er vedhæftet præcis tre produktbilleder sammen med templaten.

De tre vedhæftede produktbilleder er de eneste produkter, der må bruges i resultatet.

VIGTIG IDENTIFIKATION AF PRODUKTER:

Produktbilledernes filnavne fungerer som produkt-ID.

Filnavnene indeholder produktnummer, produktnavn og produktkode.

Formatet er:

B1-Squishy-ProduktNavn-ProduktKode.jpg

Eksempel:

B1-Squishy-Butter-HDK23D.jpg

Brug filnavnet som en vigtig reference til at identificere hvert fysisk produkt.

Uploadrækkefølgen er kun en reference og må ikke bruges som eneste identifikation.

Hvis billederne bliver uploadet i en anden rækkefølge, skal filnavnet bruges til at identificere det korrekte produkt.

Hvis B1-Squishy-Butter-HDK23D.jpg eksempelvis bliver uploadet som det tredje billede, er det stadig Produkt 1.

Brug samtidig produktets faktiske visuelle udseende til at kontrollere identiteten.

Hvis produktnavnet og produktets visuelle udseende tydeligt beskriver det samme produkt, skal dette bruges som ekstra bekræftelse.

Brug kun de tre faktiske produkter fra de uploadede billeder.

Du må ikke bruge produkter fra andre billeder.

Du må ikke vælge andre produkter.

Du må ikke generere nye produkter.

Du må ikke opfinde produkter.

Du må ikke ændre produkternes design.

Du må ikke ændre produkternes farver.

Du må ikke ændre produkternes form.

Du må ikke tilføje detaljer til produkterne.

Du må ikke fjerne detaljer fra produkterne.

Bevar hvert produkts originale identitet.

Bevar hvert produkts originale form.

Bevar hvert produkts originale farve.

Bevar hvert produkts originale materiale.

Bevar hvert produkts originale detaljer.

Bevar hvert produkts originale mønster.

Bevar hvert produkts originale udseende.

Hvis produktbillederne har en original baggrund, skal baggrunden fjernes forsigtigt, så kun det originale produkt placeres på templaten.

Produktets kanter skal være rene og naturlige.

Placér de tre produkter elegant som en harmonisk lille gruppe i det store blanke område under teksten.

Produkt 1 skal placeres til venstre.

Produkt 2 skal placeres til højre.

Produkt 3 skal placeres let foran og mellem Produkt 1 og Produkt 2.

Produkterne må gerne overlappe hinanden en smule.

Sørg for at alle tre produkter er tydelige og lette at genkende.

Sørg for en naturlig størrelse på alle tre produkter.

Sørg for god luft omkring gruppen.

Ingen af produkterne må dække teksten.

Ingen af produkterne må dække de vigtigste dekorative elementer.

Giv hvert produkt en realistisk, blød og naturlig skygge på baggrunden.

Skyggerne skal få produkterne til at se fysisk placeret på designet ud.

Resultatet skal ligne et ægte æstetisk produktfoto integreret direkte i den eksisterende feminine kawaii template.

Resultatet må ikke ligne et kunstigt AI genereret produkt.

Ingen ekstra produkter.

Ingen ekstra tekst.

Ingen priser.

Ingen produktnavne.

Ingen produktkoder.

Ingen nye dekorative elementer.

Ingen TikTok ikoner.

Ingen TikTok interface.

Ingen ændring af templateets design.

Ingen ændring af templateets typografi.

Ingen ændring af templateets farver.

Ingen ændring af templateets dekorative elementer.

Formatet skal være præcis 9:16.

Lever ét færdigt vertikalt forsidebillede i 9:16.

Høj opløsning.

Skarpe og tydelige produkter.

De tre produkter skal være de samme produkter som i de tre vedhæftede produktbilleder.
`;

function createSafeFilePart(value) {
  return String(
    value || "product"
  )
    .trim()
    .replace(
      /[^a-zA-Z0-9æøåÆØÅ]+/g,
      "-"
    )
    .replace(
      /^-+|-+$/g,
      "");
}

function createSafeCode(value) {
  return String(
    value || "UNKNOWN"
  )
    .trim()
    .replace(
      /[^a-zA-Z0-9]+/g,
      "");
}

function createProductFilename(
  product,
  productNumber
) {
  const productName =
    createSafeFilePart(
      product["Product Name"]
    );

  const productCode =
    createSafeCode(
      product["Product Code"]
    );

  return `B${productNumber}-Squishy-${productName}-${productCode}.jpg`;
}

export function createPrompt(
  products
) {
  if (
    !products ||
    products.length !== 3
  ) {
    throw new Error(
      "Prompten skal bruge præcis 3 produkter."
    );
  }

  const product1Filename =
    createProductFilename(
      products[0],
      1
    );

  const product2Filename =
    createProductFilename(
      products[1],
      2
    );

  const product3Filename =
    createProductFilename(
      products[2],
      3
    );

  return PROMPT_TEMPLATE
    .replace(
      "{{PRODUCT_1_FILENAME}}",
      product1Filename
    )
    .replace(
      "{{PRODUCT_1_NAME}}",
      products[0][
        "Product Name"
      ]
    )
    .replace(
      "{{PRODUCT_1_CODE}}",
      products[0][
        "Product Code"
      ]
    )
    .replace(
      "{{PRODUCT_1_PRICE}}",
      `${products[0]["Price"]} ${products[0]["Currency"]}`
    )
    .replace(
      "{{PRODUCT_2_FILENAME}}",
      product2Filename
    )
    .replace(
      "{{PRODUCT_2_NAME}}",
      products[1][
        "Product Name"
      ]
    )
    .replace(
      "{{PRODUCT_2_CODE}}",
      products[1][
        "Product Code"
      ]
    )
    .replace(
      "{{PRODUCT_2_PRICE}}",
      `${products[1]["Price"]} ${products[1]["Currency"]}`
    )
    .replace(
      "{{PRODUCT_3_FILENAME}}",
      product3Filename
    )
    .replace(
      "{{PRODUCT_3_NAME}}",
      products[2][
        "Product Name"
      ]
    )
    .replace(
      "{{PRODUCT_3_CODE}}",
      products[2][
        "Product Code"
      ]
    )
    .replace(
      "{{PRODUCT_3_PRICE}}",
      `${products[2]["Price"]} ${products[2]["Currency"]}`
    );
}

export function createPromptSet(
  products
) {
  if (
    !products ||
    products.length !== 9
  ) {
    throw new Error(
      "Der skal bruges præcis 9 produkter."
    );
  }

  return [
    createPrompt(
      products.slice(0, 3)
    ),
    createPrompt(
      products.slice(3, 6)
    ),
    createPrompt(
      products.slice(6, 9)
    )
  ];
}

export function createCoverPrompt() {
  const coverTitle =
    createCoverTitle();

  return COVER_PROMPT.replace(
    "{{COVER_TITLE}}",
    coverTitle
  );
}
