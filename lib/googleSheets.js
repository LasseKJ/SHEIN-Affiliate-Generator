...
export async function getProducts() {
  ...
  const activeProducts = products.filter((product) => {
    return product["Active"]?.toUpperCase() === "YES";
  });

  if (activeProducts.length < 9) {
    throw new Error(
      `Der er kun ${activeProducts.length} aktive produkter. Der skal være mindst 9.`
    );
  }

  const shuffledProducts = [...activeProducts].sort(
    () => Math.random() - 0.5
  );
